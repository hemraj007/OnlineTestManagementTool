...
#echo <<<_EOT  to use this, uncomment at line 125 and remove this -> ?>
	...
	<?php  
	  ...
	  while($row=mysqli_fetch_assoc($result)){
        echo"<tr><td><input type='checkbox' selected='true'></td>";
	  	echo "<td>".$i++."</td> ";
	  	echo "<td>". $row['ques']."</td>";
	  	echo"<td>".$row[$row['ans']]."</td>";
      echo"<td>".$row['qno']."</td>";
      echo"<td>".$num_ques."</td>";
      echo"<td>".$exam_id."</td>;</tr>";
      $qno = $row['qno'];

      require('../Login/dbh.php');
      $sql=mysqli_query($conn,"INSERT INTO exams(examid,testid,qno,Noofques) VALUES ('$exam_id','$subject','$qno','$num_ques')");
    ...
    <form method="POST" action="../Login/admin.php">
      <input type="submit" name="Add to Exam" >
    </form>
...
<!-- CREATE TABLE answers ( Userid int(13), Testid int(4), qno int(11), answer int(3), PRIMARY KEY(Userid,Testid), FOREIGN KEY(Userid) REFERENCES login(ID), FOREIGN KEY(Testid) REFERENCES exams(testid), FOREIGN KEY(qno) REFERENCES questionbank(qno) )


CREATE TABLE result ( Userid int(13), Testid int(4), score int(3), PRIMARY KEY(Userid,Testid), FOREIGN KEY(Userid) REFERENCES login(ID), FOREIGN KEY(Testid) REFERENCES exams(testid) )


CREATE TABLE exams ( testid int(4), Noofques int(3), PRIMARY KEY(testid) )
 -->
