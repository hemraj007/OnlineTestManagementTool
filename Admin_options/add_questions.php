...
if($_SERVER['REQUEST_METHOD']=='POST') {
     ...
     $sql=mysqli_query($conn,"INSERT INTO questionbank(testid, ques,a,b,c,d,ans) VALUES ('$testid','$question','$a','$b','$c','$d','$answer' )");
     if($sql = true){
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      $newtext = wordwrap($row['ques'] , 50, "<br/>", true);
        echo $newtext."</p><pre><br>(a) ".wordwrap($row['a'] , 50, "<br/>", true)."<br>(b) ".wordwrap($row['b'] , 50, "<br/>", true)."<br>(c) ".wordwrap($row['c'] , 50, "<br/>", true)."<br>(d) ".wordwrap($row['d'] , 50, "<br/>", true).
              "<br/><br/></pre>Answer Stored: <pre>".$row['ans'].") ". wordwrap($row[$row['ans']] , 50, "<br/>", true)."</pre>";
    }
} else {
    echo "0 results";
}
...
CREATE TABLE result (
    ...
    FOREIGN KEY(Testid) REFERENCES exams(testid)
    ); -->